...
from datasets import load_dataset
total_data = []
ds = load_dataset("bzantium/KMMLU-Pro", split='test')
subset = ds.shuffle(seed=42).select(range(length_of_dataset))
total_data.extend(subset['question'])   
...
haerae_data = []
for name in ['General Knowledge', 'History', 'Loan Words', 'Reading Comprehension', 'Rare Words', 'Standard Nomenclature']:
    ds = load_dataset("HAERAE-HUB/HAE_RAE_BENCH_1.0", name, split='test')
    haerae_data.extend(ds['question'])
random.shuffle(haerae_data)
total_data.extend(haerae_data[:length_of_dataset])

kobest_data = []
for name in ['boolq', 'copa', 'hellaswag', 'sentineg', 'wic']:
    ds = load_dataset("skt/kobest_v1", name, split='test')
    if name == ' boolq':
        paragraph = ds['paragraph']
        question = ds['question']
        for i in range(min(len(paragraph), length_of_dataset)):
            kobest_data.append(f"{paragraph[i]}\n{question[i]}")
    elif name == 'copa':
        kobest_data.extend(ds['premise'][:length_of_dataset])
    elif name == 'hellaswag':
        kobest_data.extend(ds['context'][:length_of_dataset])
    elif name == 'sentineg':
        kobest_data.extend(ds['sentence'][:length_of_dataset])
    
random.shuffle(kobest_data)
total_data.extend(kobest_data)

with open('kr_benchmark_data.jsonl', 'w', encoding='utf-8') as f:
